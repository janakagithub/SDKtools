package compare_models_multiple::compare_models_multipleClient;
...
compare_models_multiple::compare_models_multipleClient
...
sub new
{
    ...
    my $self = {
	client => compare_models_multiple::compare_models_multipleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a compare_models_multiple.workspace_name
$contigset_id is a compare_models_multiple.contigset_id
$return is a compare_models_multiple.CountContigsResults
...
# Authentication: required
	...
	method => "compare_models_multiple.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "compare_models_multiple.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for compare_models_multiple::compare_models_multipleClient\n";
    }
    if ($sMajor == 0) {
        warn "compare_models_multiple::compare_models_multipleClient version is $svr_version. API subject to change.\n";
    }
}
...
package compare_models_multiple::compare_models_multipleClient::RpcClient;
