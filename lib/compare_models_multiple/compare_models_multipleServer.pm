package compare_models_multiple::compare_models_multipleServer;
...
our %return_counts = (
        'count_contigs' => 1,
        'count_contigs_async' => 1,
        'count_contigs_check' => 1,
        'version' => 1,
);

our %method_authentication = (
        'count_contigs' => 'required',
        'count_contigs_async' => 'required',
        'count_contigs_check' => 'required',
);
...
our %async_run_methods = (
        'count_contigs_async' => 'compare_models_multiple.count_contigs',
);

our %async_check_methods = (
        'count_contigs_check' => 'compare_models_multiple.count_contigs',
);
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'compare_models_multiple';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = compare_models_multiple::compare_models_multipleServerContext->new($self->{loggers}->{userlog},
...
{
    # Service compare_models_multiple requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for compare_models_multiple but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = compare_models_multiple::compare_models_multipleServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service compare_models_multiple.");
    }
...
package compare_models_multiple::compare_models_multipleServerContext;

use strict;

=head1 NAME

compare_models_multiple::compare_models_multipleServerContext
...
package compare_models_multiple::compare_models_multipleServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = compare_models_multiple::compare_models_multipleServer->new(
